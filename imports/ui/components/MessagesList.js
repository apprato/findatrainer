...
const handleNavigation = (_id) => {
  browserHistory.push(`/messages/${_id}`);
}

const handleNavigationPager = (selected) => {
  window.location.href = '/messages/page/' + selected;
}
  ...
  render() {
    ...
    return (<div className="Messages">
      ...
      <div className="Messages-list">
        { messages.length > 0 ? messages.map(({_id, dob, title, sex, weight, weightMetric, height, heightMetric, firstName, lastName}) => (
          <Panel>
            <Row>
              <Col xs={ 8 } sm={ 10 }>
                <a href={ "/messages/" + _id } key={ _id } onClick={ () => handleNavigation(_id) }>
